	...
	<div class="wrapper">
		...
		<div class="list_all">
			...
			<div class="user-list">
				...
				<li class="user-wx" v-for="user in wxUserList" :key="user.id">
					<a>{{ user.id }}</a>
				...
				<div class="scrollable-container">
					<ul class="item">
						<li class="manager-item-left" v-for="webuser in filteredLeftUserList" :key="webuser.name">
							...
							<div class="edit" >
								<button class="edit-admin" @click="checkEditMagType(webuser)">修改</button>
							</div>
						</li>
					</ul>
    ...
    export default {
        ...
        methods: {
			...
			addManager() {
				if(this.addType!=3){
					// 发送添加管理员请求
					axios.post('http://47.98.58.79:8080/Manager', this.newManager)
					.then(response => {
					//处理添加成功的逻辑
					...
					// 发送添加管理员请求
					axios.put('http://47.98.58.79:8080/Manager', this.newManager)
					.then(response => {
					// 处理添加成功的逻辑
				...
				deleteManagers() {
					...
					for(let i = 0; i<this.selectedManagers.length;i++){
						...
						axios.delete('http://47.98.58.79:8080/Manager',{params:this.deleteList})
				...
				uploadImage() {
					...
					if (file.size > 10 * 1024 * 1024) { // 将文件大小转换为字节
						...
						reader.onload = () => {
							...
							// 发送数据给后端的逻辑
							fetch('http://47.98.58.79:8080/Banner', {
								method: 'POST',
								headers: {
				...
				deleteImage(){
					axios.delete('http://47.98.58.79:8080/Banner')
						.then(response => {
