...
<div class="wrapper">
	...
	<div class="search-box">
		...
		<input type="text" id="searchInput" placeholder="请输入快递单号" v-model="searchKeyword" @keyup.enter="filterByKeyword">
	...
	<div class="op-list">
		...
		<!--状态选择-->
		<div class="switcher">
     		<select id="status-switcher" @change="filterByStatus">
  ...
  methods: {
	...
	filterByStatus() {
		...
		this.items.forEach(item => {
			item.show = this.filteredItems.includes(item);
		});
	...
	fetchDataFromBackend() {
		axios.get('http://47.98.58.79:8080/Record/getall')
		.then(response => {
	...
	filterByKeyword() {
		// 使用 searchKeyword 进行关键词搜索
		this.filteredItems = this.items.filter(item => {
		// 假设需要搜索的字段是 tracking_number
		return item.tracking_number.toLowerCase().includes(this.searchKeyword.toLowerCase());
		});
	...
	filterByStatusAndKeyword() {
		...
		// 首先应用状态筛选
		let filteredByStatus = this.items.filter(item => {
			return selectedStatus === '' || item.update_state === selectedStatus;
		});
		...
		// 更新列表项的显示状态
		this.items.forEach(item => {
			item.show = filteredByStatus.includes(item);
		});

		// 如果过滤后的数组为空，则显示“无数据”
		this.noData = filteredByStatus.length === 0;
		},
